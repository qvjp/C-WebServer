...
void doit(int fd)
{
    ...
    if (!strcasecmp(method, "HEAD")) {
        ...
        clienterror(fd, method, "501", "Not Implemented", "INLOW does not implement this method");
        return;
    }
    ...
    if (stat(filename, &sbuf) < 0) {
        clienterror(fd, filename, "404", "Not found", "INLOW couldn't read the file");
        return;
    }

    if (is_static) {
        if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {
            clienterror(fd, filename, "403", "Forbidden", "INLOW couldn't read the file");
            return;
        }
        //printf("yes\n");
        ...
        if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) {
            clienterror(fd, filename, "403", "Forbidden", "INLOW couldn't run the CGI program");
            return;
        }
        if (!post)
...
void clienterror(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg)
{
    ...
    /* 生成HTTP响应主体 */
    sprintf(body, "<html><head><meta charset='UTF-8'><title>INLOW Error</title></head>");
    sprintf(body, "%s<body bgcolor='#ffffff'>\r\n", body);
    sprintf(body, "%s<h2>%s: %s</h2>\r\n", body, errnum, shortmsg);
    sprintf(body, "%s<p>%s: %s</p>\r\n", body, longmsg, cause);
    sprintf(body, "%s<hr><em>The INLOW Web server</em></body></html>\r\n", body);  /* <hr>标签是水平分割线 */
...
void serve_static(int fd, char *filename, int filesize, int is_head_method)
{
    ...
    sprintf(buf, "%sServer: INLOW Web Server\r\n",  buf);
